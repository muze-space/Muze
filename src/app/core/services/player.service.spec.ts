...
describe('PlayerService', () => {
  ...
  it('toggleShuffle() flips the shuffle signal', () => {
    expect(service.shuffle()).toBe(false);
  ...
  it('next()/previous() still traverse every track in the queue when shuffle is on', () => {
    service.play(trackA, [trackA, trackB, trackC]);
    service.toggleShuffle();

    const visited = new Set([trackA.id]);
    service.next();
    visited.add(service.currentTrack()!.id);
    service.next();
    visited.add(service.currentTrack()!.id);
  ...
  it('cycleRepeatMode() cycles through off -> all -> one -> off', () => {
  ...
  it('onTrackEnded() advances to the next track when repeat is off and mid-queue', () => {
    service.play(trackA, [trackA, trackB, trackC]);
    service.onTrackEnded();
    expect(service.currentTrack()).toBe(trackB);
  });

  it('onTrackEnded() pauses at the end of the queue when repeat is off', () => {
  ...
  it('onTrackEnded() wraps around to the first track when repeat is set to all', () => {
    service.play(trackC, [trackA, trackB, trackC]);
    service.cycleRepeatMode();
    service.onTrackEnded();
